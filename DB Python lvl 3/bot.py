...
# event ini akan muncul ketika bot sudah siap digunakan
@bot.event
async def on_ready():
    print(f'Bot is ready. Logged in as {bot.user}')

# command ini untuk memulai interaksi dengan bot dan menampilkan informasi dasar!
...
# command ini untuk menampilkan informasi tentang perintah yang tersedia!
...
# command ini untuk menambahkan project baru! Jangan lupa menambahkan status!
...
# command ini untuk menampilkan SEMUA project milik user!
...
# command ini untuk menambahkan skills ke project yang sudah ada!
...
async def skills(ctx):
    ...
    if projects:
        ...
        await ctx.send("\n".join(projects)) # Menampilkan daftar proyek

        def check(msg): # Fungsi untuk memeriksa pesan dari pengguna yang sama dan di saluran yang sama
            return msg.author == ctx.author and msg.channel == ctx.channel
        ...
        if skill.content not in skills: # Memeriksa apakah keterampilan ada dalam daftar
            await ctx.send('Sepertinya kamu memilih keterampilan yang tidak ada dalam daftar, silakan coba lagi!) Pilih keterampilan')
            return

        manager.insert_skill(user_id, project_name.content, skill.content) # Menambahkan keterampilan ke proyek
        await ctx.send(f'Keterampilan {skill.content} telah ditambahkan ke proyek {project_name.content}')
    else:
        await ctx.send('Kamu belum memiliki proyek!\nKamu dapat menambahkannya menggunakan perintah !new_project')

# Fungsi command ini untuk menghapus project yang sudah ada!
@bot.command(name='delete')
async def delete_project(ctx):
    user_id = ctx.author.id
    projects = manager.get_projects(user_id)
    if projects: # Memeriksa apakah pengguna memiliki proyek
        projects = [x[2] for x in projects]
        ...
        project_name = await bot.wait_for('message', check=check)
        if project_name.content not in projects: # Memeriksa apakah proyek ada dalam daftar
 ...
 # fungsi command ini untuk mengubah atau menambahkan data ke project yang sudah ada!
...
async def update_projects(ctx):
    ...
    if projects:
        ...
        if attribute.content not in attributes: # Memeriksa apakah atribut ada dalam daftar
            await ctx.send("Sepertinya kamu membuat kesalahan, silakan coba lagi!")
            return 

        if attribute.content == 'Status': # Jika atribut yang dipilih adalah 'Status', tampilkan daftar status yang tersedia
...
# event ini akan menangani pesan yang dikirim oleh user atau untuh menambahkan TOKEN 
bot.run(TOKEN)